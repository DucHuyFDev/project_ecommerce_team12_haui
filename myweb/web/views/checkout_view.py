...
def checkout(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Kiểm tra nếu là thanh toán VNPay, chuyển khoản ngân hàng hoặc thẻ tín dụng
            ten_pttt_lower = thanh_toan.TenPTTT.lower()
            is_vnpay = (
                'vnpay' in ten_pttt_lower or 
                'chuyển khoản' in ten_pttt_lower or 
                'chuyen khoan' in ten_pttt_lower or
                'ngân hàng' in ten_pttt_lower or
                'ngan hang' in ten_pttt_lower or
                'thẻ tín dụng' in ten_pttt_lower or
                'the tin dung' in ten_pttt_lower or
                'thẻ tín dụng' in ten_pttt_lower or
                'credit card' in ten_pttt_lower or
                'visa' in ten_pttt_lower or
                'mastercard' in ten_pttt_lower
            )
            ...
            don_hang = DonHang.objects.create(
                ...
                trangThaiGH='Chờ thanh toán' if is_vnpay else 'Chờ xử lý',
            ...
            # Nếu là VNPay: redirect đến trang thanh toán VNPay (chưa trừ tồn kho, chưa xóa giỏ)
            if is_vnpay:
                # Lưu order_id vào session để sau khi thanh toán xong có thể lấy lại
                request.session['pending_order_id'] = don_hang.id
                # Redirect đến VNPay payment với tham số
                vnpay_url = reverse('vnpay:payment') + f'?order_id={don_hang.id}&amount={int(final_money_to_save)}&order_desc=Thanh+toan+don+hang+{don_hang.id}'
                return redirect(vnpay_url)
            
            # Nếu không phải VNPay: xử lý như cũ (trừ tồn kho, xóa giỏ)
            for item in cart_items:
                # Trừ tồn kho

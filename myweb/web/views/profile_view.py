...
from ..models import TaiKhoan, DonHang, DiaChi, ChiTietDonHang, Payment_VNPay
...
def payment_history(request):
    """Hiển thị lịch sử thanh toán VNPay của người dùng"""
    kh_id = request.session['khach_hang_id']
    khach_hang = TaiKhoan.objects.get(pk=kh_id)
    
    # Lấy danh sách đơn hàng của người dùng
    don_hangs = DonHang.objects.filter(TaiKhoan=khach_hang).values_list('id', flat=True)
    
    # Lấy lịch sử thanh toán VNPay của các đơn hàng này, sắp xếp mới nhất lên đầu
    try:
        payment_history_list = Payment_VNPay.objects.filter(order_id__in=don_hangs).order_by('-id')
    except Exception as e:
        # Nếu bảng chưa tồn tại, hiển thị thông báo lỗi
        from django.contrib import messages
        messages.error(request, f"Lỗi: Bảng lịch sử thanh toán chưa được tạo. Vui lòng chạy: python manage.py migrate")
        payment_history_list = []
    
    # Thêm thông tin đơn hàng vào mỗi payment
    for payment in payment_history_list:
        try:
            payment.don_hang = DonHang.objects.get(pk=payment.order_id)
        except DonHang.DoesNotExist:
            payment.don_hang = None
        
        # Xác định trạng thái thanh toán
        if payment.vnp_ResponseCode == '00':
            payment.status_text = 'Thành công'
            payment.status_class = 'success'
        else:
            payment.status_text = 'Thất bại'
    ...
    return render(request, 'profile/payment_history.html', context)